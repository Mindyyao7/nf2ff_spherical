...
%% Initialize Variables of the input data
num_rows = 21;
num_columns = 21;
x_min = -0.025;
x_max = 0.025;
dx = 0.0025; % Sample spacing in the x direction
dy = 0.0025; % Sample spacing in the y direction
x_datapath = 'data_x'; % Define folder name for x_data
y_datapath = 'data_y'; % Define folder name for y_data

IWANTFIGURES = 1;

file = dir(fullfile('./', x_datapath, '*.s2p'));
fistFile = fullfile(x_datapath, file(1).name);
sdata = sparameters(fistFile); % HFSS simulation_change it everytime when switching
c=299792458; % Speed of light in vacuum [m/s]
freq=sdata.Frequencies; % Measured frequency points [Hz]
num_frequencies=length(freq);
f_start=freq(1);
f_stop=freq(num_frequencies);
df=(f_stop-f_start)/(num_frequencies-1);
dt=1/(num_frequencies*df);
t=(0:num_frequencies-1)*dt;
x=c*t;

for k = 1:length(file)
    fileName = fullfile(x_datapath, file(k).name);
    % Define the regular expression pattern to extract X and Y positions
    pattern = 'X_([-+]?\d*\.?\d+)_Y_([-+]?\d*\.?\d+)\.s2p';
    % Use regexp to match the pattern in the file name
    matches = regexp(file(k).name, pattern, 'tokens', 'once');
    X_position = str2double(matches{1});
    Y_position = str2double(matches{2});
    row = int16((X_position - x_min) / dx + 1);
    col = int16((Y_position - x_min) / dy + 1);
    for f_index = 1:num_frequencies
        data_points_x(row, col, f_index) = sparameters(fileName).Parameters(2, 1, f_index); % Separate the data by frequency
    end
end

X_coord = X_position

file = dir(fullfile('./', y_datapath, '*.s2p'));
for k = 1:length(file)
    fileName = fullfile(y_datapath, file(k).name);
    % Define the regular expression pattern to extract X and Y positions
    pattern = 'X_([-+]?\d*\.?\d+)_Y_([-+]?\d*\.?\d+)\.s2p';
    % Use regexp to match the pattern in the file name
    ...
    X_position = str2double(matches{1});
    Y_position = str2double(matches{2});
    row = int16((X_position + x_max) / dx + 1);
    col = int16((Y_position + x_max) / dy + 1);
    for f_index = 1:num_frequencies
        data_points_y(row, col, f_index) = sparameters(fileName).Parameters(2,1,f_index); % Separate the data by frequency
    end
end
